...
本记录对应 `rtl-rs-check 0.4.0`（提交 `625ba89`），覆盖 `module_rules`、`has_rs_cfg_en`、`step_parameters`、未登记模块默认规则、逐实例有效拍贡献、report schema v3 和 GUI“模块规则库”页。Windows 与 Linux VM 结果均来自 2026-07-25 对该提交的实际执行；2026-07-24 的历史数据不作为本版本通过证据。
...
```text
Ran 126 tests in 4.364s
OK (skipped=22)
```

22 项 skip 均为 Windows 不适用的 Linux Bash/X11 或 POSIX 进程组用例；其余 104 项通过。已执行用例覆盖 `step=0`、隐式默认规则、显式规则优先、默认规则的 step mismatch、缺少/非零 `RS_CFG_EN`、Excel 未填 `假门控`、无 step parameter、单/多 parameter、参数缺失、`null`、X/Z、非法值、report v3 结构校验、GUI 规则保存和配置路径同步逻辑。

同一工作区执行 `python -m build` 成功生成 `rtl_rs_check-0.4.0` wheel 和源码包。
...
另行执行 `--default-rule` 可见 GUI smoke，未登记的 `rs_default_pipe` 不在临时规则库中；report v3 和界面均验证最终规则为 `has_rs_cfg_en=true`、`step_parameters=[]`，2 个实例的贡献为 `[1,1]`、有效/期望拍为 `2/2`：

```text
GUI_SMOKE_PASS: state=PASS rows=行数 1 errors=错误 0 warnings=警告 0 mode=offline case=default-rule iterations=1 ... rule=unregistered-default has-rs-cfg-en=true step-parameters=[] physical=2 effective=2 contributions=1,1 schemas=report-v3/inventory-v2
```

本地离线反例的 GUI 状态为 FAIL、2 errors、0 warning。VM 端另行完成了 `980×680` 的规则库页和结果页布局验收，见下一节。

## 2. Linux VM 已验证

环境：CentOS 7.9，Python 3.8.13，GCC/G++ 11.2.1，Verdi/NPI O-2018.09-SP2，`NPI_PLATFORM=LINUX64`。VM 从提交 `625ba892974824a9b44da8377e72246212d316f9` 的完整 Git bundle 检出干净工作树；测试副本位于 `/root/suhua_rs_tool_default_rule_20260725`，最终 fresh-KDB 产物目录为 `output/verdi_gui_test.tkGvMkt4`。

完整脚本实际执行：

```bash
cd /root/suhua_rs_tool_default_rule_20260725
# 非交互 shell 先加载本站正常运行 Verdi 所需的标准环境。
bash scripts/test_vm_verdi_gui.sh
```

首次从未加载站点 Verdi 环境的非交互 SSH shell 启动时，Verdi 因无法取得 license 而在 GUI 出现前退出，脚本正确返回失败且只清理本次 fresh KDB 进程。加载该 VM 的标准 Verdi 环境后重新完整执行，得到下表结果；脚本没有增加 license server 连通性预检，也没有终止既有 Verdi。

| 项目 | 实际结果 | 状态 |
|---|---|---|
| Linux 全量测试 | `Ran 126 tests in 4.023s`，`OK`，无 skip | PASS |
...
| 未登记模块默认规则 GUI | 模块不在规则库；`has_rs_cfg_en=true`、空 `step_parameters`、physical/effective=`2/2`、贡献 `[1,1]` | PASS |
...
| 10,000 行 GUI 负载 | 可见 Tk 窗口完成 10,000 行渲染，10,000 行 PASS、0 error/0 warning | PASS |
...
`0.4.0` 已经通过 Windows 离线回归和默认规则 GUI smoke，以及 Linux VM 上的 fresh KDB、真实 NPI、Verdi GUI、在线正负例、未登记模块默认规则 GUI、三轮在线 GUI、100 轮稳定性、10,000 行压力和两个最小窗口布局验收。当前结论仅覆盖仓库示例与记录的工具版本；生产设计仍需使用其正式流程生成的 elaborated KDB 运行同一检查。
